...
from tensorflow.keras.layers import Dense, Conv1D, MaxPooling1D, Dropout, Flatten, Embedding, AveragePooling1D
...
Model_name = "LENET_CrossDividesKL" #Change the loss function name e.g, CrossPlusKL
...
model.add(Conv1D(filters=128, kernel_size=3, padding='same', activation='relu', strides=2))
...
model.add(Dropout(0.3))
model.add(Dense(30, activation='softmax'))  # Use softmax for multi-class classification
...
print('Training..')
model.fit(X_train, y_train, epochs=90, batch_size=64,verbose=1,validation_data=(X_test,y_test))
...
final_model_save_path = os.path.join(MODEL_PATH, "Models/With_loss/" + Model_name + "_FINAL.h5")
